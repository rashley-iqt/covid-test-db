...
func insertRawPerformanceData( dxId uuid.UUID,  sourceOfPerfData string, sourceDisplayName string,
    ...
    err := db.Exec(
    `INSERT INTO covid_diagnostics.diagnostic_types (diagnostic_id, source_of_perf_data, source_display_name,
    clinical_lod, ppa, npa, performance_target, performance_notes,
    created_by, created, updated_by, updated) 
    VALUES (@diagnostic_id, @source_of_perf_data, @source_display_name,
    @clinical_lod, @ppa, @npa, @performance_target, @performance_notes,
    @created_by, @created, @updated_by, @updated);`,
    map[string]interface{}{
        "diagnostic_id": dxId, 
        "source_of_perf_data": sourceOfPerfData, 
